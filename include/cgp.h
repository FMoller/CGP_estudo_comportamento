...
*/
bdd get_from_token(const char *str);
...
*/
bdd analyze_mult(const char *str);
...
* @return the bdd of the boolean expression
*/
bdd analyze_sum(const char *str);
...
* @return none
*/
void table_constructor(const char* filename);
...
* @return none
*/
void set_genes_not_used(Individual *individual, int num_gates);
...
* @return none
*/
void set_nots_on_individual(Individual *individual);
...
*/
int get_input_position(const char *str);
...
*/
int get_next_free_position(Individual *individual);
...
*/
int parse_expression_only_ands(Individual *individual, const char *str);
...
*/
int parse_expression_only_ors(Individual *individual, const char *str);
...
*/
int parse_expression_ands_ors(Individual *individual, const char *str);
...
* @return the position into the genotype that the last AND/OR gate is set.
*/
void sow_population(Individual *individual, const char *filename, int num_gates);
...
* @return none
*/
void individual_constructor(Individual *individual);
...
* @return none
*/
void randomize_inputs_low_lb(Individual *individual, int row, int col, int num_input);
...
* @return none
*/
void randomize_inputs(Individual *individual, int row, int col, int num_input);
...
* @return none
*/
void initialize_individual(Individual *individual, int *gates);
...
* @return none
*/
void mutate_gene(Individual *individual, int *gates, int gene_pos);
...
* @return none
*/
void mutate_output(Individual *individual, int gene_pos);
...
* @return none
*/
void point_mutation(Individual *individual, int *gates);
...
* @return none
*/
void sam(Individual *individual, int *gates);
...
* @return none
*/
void finds_active_genes_for_each_output(Individual *individual, int output);
...
* @return none
*/
void finds_individual_active_genes(Individual *individual);
...
* @return none
*/
void print_boolean_expression_for_each_output(Individual *individual, int output);
...
* @return none
*/
void print_boolean_expression(Individual *individual);
...
*/
int get_max_depth_for_each_output(Individual *individual, int output);
...
* @return the max depth found
*/
void get_max_depth(Individual *individual);

void get_max_depth_from_inputs(Individual *individual, int gene, int depth, int *inputs);

int get_max_depth_from_outputs(Individual *individual, int gene, int sorted_gene);
...
* @return none
*/
void print_post_optimization_data(Individual *individual);
...
*/
bdd get_individual_bdd_for_each_output(Individual *individual, int output);
...
* @return none
*/
void clear_bddref(Individual *individual, int output);
...
* @return none
*/
void initialize_population(Individual *population, int *gates);
...
* @return none
*/
void apply_PM(Individual *population, int *gates);
...
* @return none
*/
void apply_SAM(Individual *population, int *gates);
...
* @return none
*/
void apply_SAM_plus_GAM(Individual *population, int *gates);
...
* @return none
*/
void evaluate_population_sat_count(Individual *population);
...
* @return none
*/
void evaluate_parent_sat_count(Individual *population);
...
* @return none
*/
void find_population_active_genes(Individual *population);
...
bdd analyze_mult(const char *str)
{
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (counter == 0)
        {
            aux = get_from_token(token);
            counter += 1;
        }
        else
        {
            aux = bdd_addref(bdd_apply(aux, get_from_token(token), bddop_and));
        }
    }
    return aux;
}
...
bdd analyze_sum(const char *str)
{
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (counter == 0)
        {
            aux = analyze_mult(token);
            counter += 1;
        }
        else
        {
            aux = bdd_addref(bdd_apply(aux, analyze_mult(token), bddop_or));
        }
    }
    return aux;
}
...
void table_constructor(const char *filename)
{
    ...
    if(file == NULL)
    {
        fprintf(out_file, "Couldn't open the file %s!\n", filename);
        exit(1);
    }
    else
    {
    ...
    for (int i = 0; i < table->num_outputs; i++)
    {
        ...
        if(fgets(buffer, 100000, file))
        {
            ...
            fprintf(out_file, "%s\n", buffer);
            table->outputs_bdd[i] = analyze_sum(buffer);
        }
    }
    fflush(out_file);
    //exit(1);
...
void set_genes_not_used(Individual *individual, int num_gates)
{
    ...
    while(1)
    {
        int temp = randomize(table->num_inputs, individual->genotype_size);
...
int get_next_free_position(Individual *individual)
{
    for(int i = table->num_inputs; i < individual->genotype_size; i++)
    {
...
int parse_expression_only_ands(Individual *individual, const char *str)
{
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (flag == 0)
        {
            input0 = get_input_position(token);
            flag = 1;
        }
        else if(flag == 1)
        {
            input1 = get_input_position(token);
            pos = get_next_free_position(individual);
...
int parse_expression_ands_ors(Individual *individual, const char *str)
{
    ...
    for (temp = (char *)str;; temp = NULL)
    {
        ...
        if (flag == 0)
        {
            input0 = parse_expression_only_ands(individual, token);
            flag = 1;
        }
        else if (flag == 1)
        {
            input1 = parse_expression_only_ands(individual, token);
            pos = get_next_free_position(individual);
...
void sow_population(Individual *individual, const char *filename, int num_gates)
{
    ...
    set_genes_not_used(individual, num_gates);
    set_nots_on_individual(individual);
    ...
    {   
        ...
        for (int i = 0; i < table->num_outputs; i++)
        {
            ...
            {
                individual->output[i] = get_input_position(buffer);
            }
            else if(strstr(buffer, "+") == NULL)
            {
                individual->output[i] = parse_expression_only_ands(individual, buffer);
            }
            else if(strstr(buffer, "*") == NULL)
            {
                individual->output[i] = parse_expression_only_ors(individual, buffer);
            }
            else if (strstr(buffer, "*") != NULL && strstr(buffer, "+") != NULL)
            {
                individual->output[i] = parse_expression_ands_ors(individual, buffer);
            }
        }
    }
...
int get_gene_row(int pos)
{
    if (pos < table->num_inputs)
    {
        fprintf(out_file, "Couldn't get gene row!\n");
        exit(1);
    }
    return (pos - table->num_inputs) % (NROW);
}
...
void individual_constructor(Individual *individual)
{
    ...
    individual->output = (int *)malloc(sizeof(int) * table->num_outputs);
    individual->score_per_output = (long int *)malloc(sizeof(long int) * table->num_outputs);
    individual->output_size = table->num_outputs;
...
void print_individual(Individual *individual)
{
    ...
    for (int row = 0; row < NROW; row++)
    {
        for (int col = 0; col < NCOL; col++)
        {
            ...
            print_gene(&individual->genotype[temp], temp + table->num_inputs);
        }
    }
    fflush(out_file);
}
...
void randomize_inputs_low_lb(Individual *individual, int row, int col, int num_input)
{
    int temp = randomize(0, table->num_inputs + LB * NROW);
    int pos = get_gene_position(row, col);

    if (temp >= 0 && temp < table->num_inputs)
    {
    ...
    else
    {
        individual->genotype[pos].input[num_input] = randomize(
            table->num_inputs + (col - LB) * NROW,
            table->num_inputs + (NROW * col));
    }
}
...
void randomize_inputs(Individual *individual, int row, int col, int num_input)
{
    ...
    {
        individual->genotype[pos].input[num_input] = randomize(0, table->num_inputs);
    }
    else if (LB >= col || LB == -1) ///aleatoriza qualquer coluna antes da entrada
    {
        individual->genotype[pos].input[num_input] = randomize(0, table->num_inputs + NROW * col);
    }
    else if (LB < col) ///aleatoriza coluna até o levels-back
    {
        randomize_inputs_low_lb(individual, row, col, num_input);
    }
}
...
void initialize_individual(Individual *individual, int *gates)
{
    ...
    individual_constructor(individual);

    for (int row = 0; row < NROW; row++) /// (For) utilizado para preencher os valores de cada nó do indivíduo
    {
        for (int col = 0; col < NCOL; col++)
        {
            randomize_inputs(individual, row, col, 0);
            randomize_inputs(individual, row, col, 1);
    ...
    for (int i = 0; i < individual->output_size; i++)
    {
        temp = randomize(0, table->num_inputs + individual->genotype_size);
...
void mutate_individual(Individual *individual, int *gates, int sorted_gene)
{
    if (sorted_gene < table->num_inputs + individual->genotype_size)
    {
        mutate_gene(individual, gates, sorted_gene);
    }
    else
    {
        mutate_output(individual, sorted_gene);
    }
}
...
void point_mutation(Individual *individual, int *gates)
{
    ...
    {
        int temp = randomize(table->num_inputs, table->num_inputs + individual->genotype_size + individual->output_size);
...
void sam(Individual *individual, int *gates)
{
    ...
    {
        int temp = randomize(table->num_inputs, table->num_inputs + individual->genotype_size + individual->output_size);
        int row = get_gene_row(temp);
        int col = get_gene_col(temp);
        int pos = get_gene_position(row, col);
        ...
        if (individual->genotype[pos].active == 1 || temp >= table->num_inputs + individual->genotype_size)
        {
            ...
            finds_individual_active_genes(individual);

            calculate_individual_sat_count(individual);
...
void gam(Individual *individual, int *gates, int worst_subgraph)
{
    while (1)
    {
        ...
        if (individual->genotype[pos].active == 1 || temp == table->num_inputs + individual->genotype_size + worst_subgraph)
        {
            mutate_individual(individual, gates, temp);
            break;
        }
...
void finds_active_genes_for_each_output(Individual *individual, int output)
{
    ...
    {
        int row = get_gene_row(output);
        int col = get_gene_col(output);
        int pos = get_gene_position(row, col);

        finds_active_genes_for_each_output(individual, individual->genotype[pos].input[0]);
        if(individual->genotype[pos].gate != 3)
            finds_active_genes_for_each_output(individual, individual->genotype[pos].input[1]);
...
void finds_individual_active_genes(Individual *individual)
{
    for (int i = 0; i < individual->output_size; i++)
    {
        finds_active_genes_for_each_output(individual, individual->output[i]);
    }
}
...
void print_boolean_expression_for_each_output(Individual *individual, int output)
{
    ...
    else
    {
        ...
        if (individual->genotype[pos].gate == 3)
        {
            ...
            print_boolean_expression_for_each_output(individual, individual->genotype[pos].input[0]);
            fprintf(out_file, ")");
        }
        else
        {
            ...
            print_boolean_expression_for_each_output(individual, individual->genotype[pos].input[0]);
            fprintf(out_file, " %s ", temp);
            print_boolean_expression_for_each_output(individual, individual->genotype[pos].input[1]);
            fprintf(out_file, ")");
        }
    }
}
...
void print_boolean_expression(Individual *individual)
{
    for (int i = 0; i < individual->output_size; i++)
    {
        print_boolean_expression_for_each_output(individual, individual->output[i]);
        fprintf(out_file, "\n\n");
    }
}
...
void get_max_depth(Individual *individual)
{
    int max_depth = get_max_depth_for_each_output(individual, individual->output[0]);
    int temp;
    for (int i = 1; i < individual->output_size; i++)
    {
        temp = get_max_depth_for_each_output(individual, individual->output[i]);
        if (temp > max_depth)
        {
            max_depth = temp;
        }
    }
    individual->depth = max_depth;
}

void get_max_depth_from_inputs(Individual *individual, int gene, int depth, int *inputs)
{
    if(gene < table->num_inputs)
    {
        if(depth > inputs[gene]) inputs[gene] = depth;
    }
    else
    {
        int row = get_gene_row(gene);
...
int get_max_depth_from_outputs(Individual *individual, int gene, int sorted_gene)
{
    if(gene == sorted_gene)
        return 0;
    else if(gene < sorted_gene || gene < table->num_inputs)
        return -1;
    else
    {
        int row = get_gene_row(gene);
...
void print_post_optimization_data(Individual *individual)
{
    ...
    //print_boolean_expression(individual);
    fflush(out_file);
}
...
bdd get_individual_bdd_for_each_output(Individual *individual, int output)
{
    ...
    else
    {
        ...
        input0 = get_individual_bdd_for_each_output(individual, individual->genotype[pos].input[0]);
        if (individual->genotype[pos].gate != 3)
            input1 = get_individual_bdd_for_each_output(individual, individual->genotype[pos].input[1]);
...
void clear_bddref(Individual *individual, int output)
{
    if(output >= table->num_inputs)
    {
        ...
        clear_bddref(individual, individual->genotype[pos].input[0]);
        if (individual->genotype[pos].gate != 3)
            clear_bddref(individual, individual->genotype[pos].input[1]);
...
void calculate_individual_sat_count(Individual *individual)
{
    ...
    for (int i = 0; i < individual->output_size; i++)
    {
        temp = get_individual_bdd_for_each_output(individual, individual->output[i]);
        ...
        bdd_delref(temp);
        clear_bddref(individual, individual->output[i]);
    }
}
...
void print_individual_RL_information(Individual *individual, int gene){
    ...
    {
        int temp = gene - individual->genotype_size - table->num_inputs;
        gene = individual->output[temp];
    }

    fprintf(out_file, "%ld\t", maxeval);
    fprintf(out_file, "%d\t%d\t", ind, gene);
    fprintf(out_file, "%ld\t%d\t%d\t", individual->score, individual->num_gates, individual->num_transistors);
    for(int i = 0; i < table->num_outputs; i++){
        fprintf(out_file, "%d\t", get_max_depth_from_outputs(individual, individual->output[i], gene));
    }
    if(gene < table->num_inputs)
    {
    
        fprintf(out_file, "%d\t%d\t%d\t", -1, -1, -1);
    ...
    else
    {
        ...
        int col = get_gene_col(gene);
        int pos = get_gene_position(row, col);
        int *inputs = (int *)malloc(sizeof(int) * table->num_inputs);

        for(int i = 0; i < table->num_inputs; i++){
            inputs[i] = -1;
        }
        get_max_depth_from_inputs(individual, gene, 0, inputs);
        fprintf(out_file, "%d\t%d\t%d\t", individual->genotype[pos].gate, individual->genotype[pos].input[0], individual->genotype[pos].input[1]);
        fprintf(out_file, "%d\t", individual->depth);
        for(int i = 0; i < table->num_inputs; i++){
...
void initialize_population(Individual *population, int *gates)
{
    ...
    {
        initialize_individual(&population[i], gates);
    }
}
...
void apply_PM(Individual *population, int *gates)
{
    ...
    {
        point_mutation(&population[i], gates);
    }
}
...
void apply_SAM(Individual *population, int *gates)
{
    ...
    for(int i = 1; i < NPOP; i++)
    {
        ind = i;
        clear_individiual_active_genes(&population[i]);
        finds_individual_active_genes(&population[i]);
        sam(&population[i], gates);
    }
}
...
void apply_SAM_plus_GAM(Individual *population, int *gates)
{
    ...
    {
        clear_individiual_active_genes(&population[i]);
        finds_active_genes_for_each_output(&population[i], population[i].output[worst_subgraph]);
        gam(&population[i], gates, worst_subgraph);
    }
}
...
void evaluate_population_sat_count(Individual *population)
{
    for (int i = 1; i < NPOP; i++)
    {
        calculate_individual_sat_count(&population[i]);
    }
}
